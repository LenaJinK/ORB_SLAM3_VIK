...
MapPublisher::MapPublisher(Atlas* pAtlas):mpAtlas(pAtlas), mbCameraUpdated(false)
{
...
void MapPublisher::PublishKeyFrames(const vector<KeyFrame*> &vpKFs)
{
    ...
    {
        cv::Mat Tcw;
        Sophus::SE3f Tcw_SE3f = vpKFs[i]->GetPose();
        Eigen::Matrix4f Tcw_Eigen = Tcw_SE3f.matrix();
        cv::eigen2cv(Tcw_Eigen, Tcw);

        cv::Mat Twc = Tcw.inv();
        cv::Mat ow;
        Eigen::Vector3f ow_Vec3f = vpKFs[i]->GetCameraCenter();
        cv::eigen2cv(ow_Vec3f, ow);
        ...
        geometry_msgs::Point msgs_o,msgs_p1, msgs_p2, msgs_p3, msgs_p4;  // notice 用于发布的点
        ...
        if(!vCovKFs.empty())
        {
            for(vector<KeyFrame*>::iterator vit=vCovKFs.begin(), vend=vCovKFs.end(); vit!=vend; vit++)
            {
                if((*vit)->mnId<vpKFs[i]->mnId)
                    continue;
                cv::Mat Ow2;
                Eigen::Vector3f Ow2_vec3f = (*vit)->GetCameraCenter();
                cv::eigen2cv(Ow2_vec3f, Ow2);
        ...
        {
            cv::Mat Owp;
            Eigen::Vector3f Owp_vec3f = pParent->GetCameraCenter();
            cv::eigen2cv(Owp_vec3f, Owp);
        ...
        for(set<KeyFrame*>::iterator sit=sLoopKFs.begin(), send=sLoopKFs.end(); sit!=send; sit++)
        {
            if((*sit)->mnId<vpKFs[i]->mnId)
                continue;
            cv::Mat Owl;
            Eigen::Vector3f Owl_vec3f = (*sit)->GetCameraCenter();
            cv::eigen2cv(Owl_vec3f, Owl);
    ...
    publisher.publish(mKeyFrames);           // notice 发布关键帧
    publisher.publish(mCovisibilityGraph);
