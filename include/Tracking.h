...
#include "pointcloudmapping.h"  // todo new

#include <mutex>
#include <unordered_set>

static int idk = 1;  // todo new 暂时没搞清楚有什么作用
...
/*    Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer, MapDrawer* pMapDrawer, Atlas* pAtlas, shared_ptr<PointCloudMapping> pPointCloud, // todo new
...
public:
    ...
    cv::Mat mImRGB;
    cv::Mat mImDepth;  // todo new adding mImDepth member to realize pointcloud view
...
protected:
    ...
    Joint::Calib *mpJointCalib; //notice 运动学参数
    ...
    std::mutex mMutexJointQueue;    // notice kinematic
    ...
    float mImuFreq;
    double mImuPer;  // notice IMU前后的timestamps可允许的差? 程序中设置为0.001
    float mJointFreq;
