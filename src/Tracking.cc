...
void Tracking::newParameterLoader(Settings *settings) {
    cout << "TRACK: newParameterLoader"<<endl;
    ...
    //missing image scaling and rectification
    mImageScale = 1.0f;
    ...
    mJointPer = 0.001; //1.0 / (double) mImuFreq;
...
bool Tracking::ParseIMUParamFile(cv::FileStorage &fSettings) {
    ...
    if (!node.empty() && node.isInt()) {
        ...
        mImuPer = 0.001; //1.0 / (double) mImuFreq;  // 经验值
    } else {
...
{
    mImRGB = imRGB;   // todo new  彩色点云
    mImGray = imRGB;  // 赋值给成员变量
    mImDepth = imD;   // todo new
//    cv::Mat imDepth = imD;  // 用 mImDepth 替换
    // 将图像转换为灰度图
        ...
        if(!mbOnlyTracking)
        {
            ...
            else  // mState != OK, 进行下面处理
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    {
                        // Step 6.4 如果IMU已经初始化,  用IMU数据预测位姿
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
                if(pCurrentMap->GetIniertialBA2()){
                    bOK = TrackLocalMapKinematic();  //notice 新增三者融合局部地图跟踪
                }
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    Optimizer::PoseOptimization(&mCurrentFrame);   // 位姿优化1 普通帧 3D-2D优化求解 重投影误差
...
bool Tracking::TrackLocalMap()
{
    ...
    else
    {
        ...
        else
        {
            ...
            if(!mbMapUpdated) //  && (mnMatchesInliers>30))  局部地图跟踪中 pose+imu 联合优化
            {
